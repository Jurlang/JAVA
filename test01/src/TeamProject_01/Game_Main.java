...
//------------------------------------------------------------------------------------------		
		...
		JButton btnNewButton = new JButton("\uC800\uC7A5"); // 저장
		...
		Update_info_Class a = new Update_info_Class(this.userNum,0,0,0, level,myitem,level);
		btnNewButton.addActionListener(new DBUpdate(a, amth, tmth));
...
class DBUpdate implements ActionListener {
	...
	Update_info_Class info;
	AutoMoney amth;
	TabMoney tmth;
	DBUpdate(Update_info_Class info, AutoMoney amth, TabMoney tmth)
	{	
	...
	public void actionPerformed(ActionEvent e) {
		...
		amth.stop = false;
		info.curmoney = amth.getCurMoney();	
		info.automoney = amth.getAutoMoney();
		info.tabmoney = tmth.tmoney;
		DBConn dbConn = DBConn.getInstance();
		dbConn.info_save(info);
		amth.stop = true;
